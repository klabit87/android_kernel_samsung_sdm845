...
void ecryptfs_clean_sdp_dek(struct ecryptfs_crypt_stat *crypt_stat)
{
	...
	if (crypt_stat->tfm) {
		...
		rc = crypto_skcipher_setkey(crypt_stat->tfm, pseudo_key,
			PSEUDO_KEY_LEN);
	...
	if (crypt_stat->flags & ECRYPTFS_DEK_SDP_ENABLED) {
		if (crypt_stat->flags & ECRYPTFS_DEK_IS_SENSITIVE) {
			...
			if (crypt_stat->engine_id < 0) {
				DEK_LOGE("get_sdp_dek: invalid engine-id(ECRYPTFS_DEK_IS_SENSITIVE:ON, engine_id:%d)\n",
...
static int ecryptfs_update_crypt_flag(struct dentry *dentry, enum sdp_op operation)
{
	...
	if (rc) {
		DEK_LOGE("Problem with ecryptfs_write_inode_size_to_metadata; "
				"rc = [%d]\n", rc);
		goto out;
	}
...
int ecryptfs_sdp_set_protected(struct dentry *dentry)
{
	...
	if (S_ISDIR(inode->i_mode)) {
	    ...
	    if (rc) {
		ecryptfs_printk(KERN_ERR, "Error computing the root IV\n");
		goto out;
	    }
...
long ecryptfs_do_sdp_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	struct ecryptfs_crypt_stat *crypt_stat =
			&ecryptfs_inode_to_private(inode)->crypt_stat;
	struct dentry *fp_dentry = ecryptfs_file_to_lower(file)->f_path.dentry;
	struct ecryptfs_mount_crypt_stat *mount_crypt_stat  =
