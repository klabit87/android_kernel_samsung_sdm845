...
enum nad_qdaf_cmd_mode {
	NAD_QDAF_MODE_EMPTY = 0,
	NAD_QDAF_MODE_START,
	NAD_QDAF_MODE_END,
	NAD_QDAF_MODE_CHECK_RUNNING,
	NAD_QDAF_MODE_END_WITH_SYNC=7,
	NAD_QDAF_MODE_SAVE_LOGS=8,
	NAD_QDAF_MODE_PANIC=9
};
#define QDAF_PARAM_START "cmdMode:1"
#define QDAF_PARAM_END "cmdMode:2"
#define QDAF_PARAM_END_WITH_SYNC "cmdMode:7"

#define QDAF_PROG "/system/bin/qnad/qdaf.sh"
#define QDAF_QMESA_LOG "/data/log/qdaf_qmesa_log.txt"
static int qdaf_cur_mode;
...
/*
static ssize_t show_nad_qdaf(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	if( qdaf_cur_mode==NAD_QDAF_MODE_START)
		return snprintf(buf, BUFF_SZ, "QDAF_IS_RUNNING");
	
	return snprintf(buf, BUFF_SZ, "QDAF_IS_NOT_RUNNING");
}
*/

static ssize_t store_nad_qdaf(struct device *dev,
			      struct device_attribute *attr,
			      const char *buf, size_t count)
{
	int idx = 0, ret = -1, cur_mode;
	char temp[NAD_BUFF_SIZE * 3];
	char nad_cmd[NAD_CMD_LIST][NAD_BUFF_SIZE];
	char *nad_ptr, *string;
	int ret_userapp;
	int wait_val = UMH_WAIT_EXEC;
	char *argv[4] = { NULL, NULL, NULL, NULL };
	char *envp[5] = { 
		"HOME=/", 
		"PATH=/system/bin/qnad:/system/bin:/system/xbin", 
	...
	ret = sscanf(nad_cmd[1], "%d\n", &cur_mode);
	/*
	if ( (ret != 1) || (cur_mode<=NAD_QDAF_MODE_EMPTY) || (cur_mode>=NAD_QDAF_MODE_MAX) ) {
		NAD_PRINT("%s : return error (cur_mode=%d)\n", __func__, cur_mode);
		return -EINVAL;;
	}
	*/
	if ( qdaf_cur_mode==cur_mode ) {
		if( qdaf_cur_mode==NAD_QDAF_MODE_START ) {
			NAD_PRINT("%s : return because qdaf.sh already has been running.\n", __func__);
			return count;
		}else if( qdaf_cur_mode==NAD_QDAF_MODE_END ) {
			NAD_PRINT("%s : return because qdaf.sh has not been running yet.\n", __func__);
			return count;
		}
	}
	qdaf_cur_mode = cur_mode;
	NAD_PRINT("%s : qdaf_cur_mode=%d\n", __func__, qdaf_cur_mode);

	argv[0] = QDAF_PROG;
	switch (qdaf_cur_mode) {
	case NAD_QDAF_MODE_START :
		NAD_PRINT("%s : qdaf will be started\n",__func__);
		argv[1] = QDAF_PARAM_START;
		break;
	case NAD_QDAF_MODE_END :
		NAD_PRINT("%s : qdaf will be ended\n",__func__);
		argv[1] = QDAF_PARAM_END;
		break;
	case NAD_QDAF_MODE_END_WITH_SYNC :
		NAD_PRINT("%s : qdaf will be ended\n",__func__);
		argv[1] = QDAF_PARAM_END_WITH_SYNC;
		wait_val = UMH_WAIT_PROC;
		break;
	case NAD_QDAF_MODE_SAVE_LOGS :
		NAD_PRINT("%s : qdaf will save log into kmsg\n",__func__);
		qdaf_save_logs();
		return count;
	case NAD_QDAF_MODE_PANIC :
	...
	if (!ret_userapp)
		...
		qdaf_cur_mode = NAD_QDAF_MODE_EMPTY;
...
//static DEVICE_ATTR(nad_qdaf, S_IRUGO | S_IWUSR, show_nad_qdaf, store_nad_qdaf);
static DEVICE_ATTR(nad_qdaf, 0200, NULL, store_nad_qdaf);
...
static int __init sec_nad_init(void)
{
	...
	ret = device_create_file(sec_nad, &dev_attr_nad_qdaf);
	if (ret) {
		pr_err("%s: Failed to create device file\n", __func__);
		goto err_create_nad_sysfs;
	}
