...
void isdbt_hw_init(void)
{
	...
	pr_err("%s, gpio_power_en = %d, gpio_rst = %d\n", __func__,
		gpio_get_value(isdbt_pdata->gpio_power_en),
...
static struct isdbt_platform_data *isdbt_populate_dt_pdata(struct device *dev)
{
...
#if defined(CONFIG_SEC_GPIO_SETTINGS)
	spi_device_node = of_parse_phandle(dev->of_node,
			"isdbt,spi_node", 0);                                  
	if (!IS_ERR_OR_NULL(spi_device_node)) {
		...
		isdbt_pinctrl = devm_pinctrl_get_select(&spi_pdev->dev
				, "sleep");
		if (IS_ERR(isdbt_pinctrl))
			pr_err("Target does not use spi pinctrl\n");
	}
#endif
	...
	pdata->gpio_en = of_get_named_gpio(dev->of_node
		, "isdbt,isdb-gpio-en", 0);
	if (pdata->gpio_en < 0)
		of_property_read_u32(dev->of_node, "isdbt,isdb-gpio-en"
			, &pdata->gpio_en);
	if (pdata->gpio_en < 0)	{
