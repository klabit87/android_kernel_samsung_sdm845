...
static uint cam_debug_ctx_req_list = 0;
...
int32_t cam_context_flush_ctx_to_hw(struct cam_context *ctx)
{
	...
	CAM_INFO(CAM_CTXT, "[%s][%d] E: NRT flush ctx",
		ctx->dev_name, ctx->ctx_id);
	...
	while (true) {
		...
		for (i = 0; i < req->num_out_map_entries; i++) {
			if (req->out_map_entries[i].sync_id != -1) {
				rc = cam_sync_signal(req->out_map_entries[i].sync_id,
					CAM_SYNC_STATE_SIGNALED_ERROR);
				if (rc == -EALREADY) {
					CAM_ERR(CAM_CTXT, "[%s][%d] Req: %llu already signalled, sync_id:%d",
						ctx->dev_name, ctx->ctx_id,
						req->request_id,
	...
	if (ctx->hw_mgr_intf->hw_flush) {
		...
		list_for_each_entry(req, &ctx->active_req_list, list) {
			flush_args.flush_req_active[flush_args.num_req_active++]
				= req->req_priv;
			CAM_INFO(CAM_CTXT, "Flush req_id:%llu dev_name: %s",
				req->request_id, ctx->dev_name);
		}
		spin_unlock(&ctx->lock);
		...
		for (i = 0; i < req->num_out_map_entries; i++) {
			if (req->out_map_entries[i].sync_id != -1) {
				rc = cam_sync_signal(req->out_map_entries[i].sync_id,
					CAM_SYNC_STATE_SIGNALED_ERROR);
				if (rc == -EALREADY) {
					CAM_ERR(CAM_CTXT, "[%s][%d] Req: %llu already signalled ctx: %pK dev_name: %s dev_handle: %d ctx_state: %d",
						ctx->dev_name, ctx->ctx_id,
	...
	CAM_INFO(CAM_CTXT, "[%s] X: NRT flush ctx", ctx->dev_name);
	...
	struct cam_flush_dev_cmd *cmd)
{
	...
	CAM_INFO(CAM_CTXT, "[%s] E: NRT flush req", ctx->dev_name);
	...
	CAM_INFO(CAM_CTXT, "[%s] X: NRT flush req", ctx->dev_name);
