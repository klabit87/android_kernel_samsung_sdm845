...
static int __cam_req_mgr_traverse(struct cam_req_mgr_traverse *traverse_data)
{
	...
	if (traverse_data->in_q->slot[curr_idx].skip_frm[tbl->pd].
		inject_delay > 0 && (traverse_data->validate_only == false)) {
		CAM_DBG(CAM_CRM, "Injecting Delay of one frame");
		apply_data[tbl->pd].req_id = -1;
		traverse_data->in_q->slot[curr_idx].skip_frm[tbl->pd].
			inject_delay--;
	...
	struct cam_req_mgr_req_queue *in_q, uint32_t trigger)
{
	...
	for (i = 0; i < link->num_devs; i++) {
		...
		if (dev) {
			...
			if (link->req.in_q->slot[idx].skip_frm[pd].
				skip_next_frame &&
				link->req.in_q->slot[idx].skip_frm[pd].
					dev_hdl == dev->dev_hdl) {
				...
				if (dev->ops && dev->ops->apply_req) {
					...
					if (rc)
						return rc;
					CAM_DBG(CAM_CRM,
						"SEND: pd %d req_id %lld",
						pd, apply_req.request_id);
					link->req.in_q->slot[idx].
						skip_frm[pd].
						skip_next_frame = false;
					link->req.in_q->slot[idx].
					skip_frm[pd].is_applied = true;
					return -EAGAIN;
				}
			}
		}
	...
	for (i = 0; i < link->num_devs; i++) {
		...
		if (dev) {
			...
			CAM_DBG_FRS(CAM_CRM, "SEND: pd %d req_id %lld",
				pd, apply_req.request_id);
#endif
			if (dev->ops && dev->ops->apply_req) {
				rc = dev->ops->apply_req(&apply_req);
	...
	struct cam_req_mgr_core_link *sync_link)
{
	...
	CAM_DBG(CAM_CRM,
		"link[%x] self_counter=%lld, other_counter=%lld, diff=%lld",
		link->link_hdl,
		link->sof_counter, sync_link->sof_counter, sync_diff);
...
int cam_req_mgr_process_add_req(void *priv, void *data)
{
	...
	if (add_req->skip_before_applying >
		link->req.in_q->slot[idx].skip_frm[tbl->pd].inject_delay) {
		link->req.in_q->slot[idx].skip_frm[tbl->pd].inject_delay =
			add_req->skip_before_applying;
		link->req.in_q->slot[idx].skip_frm[tbl->pd].dev_hdl =
			add_req->dev_hdl;
		link->req.in_q->slot[idx].skip_frm[tbl->pd].
...
#if defined(CAMERA_FRS_DRAM_TEST_DBG_LOG)
	CAM_DBG_FRS(CAM_CRM, "link_hdl %x frame_id %lld, trigger %x",
		trigger_data->link_hdl,
		trigger_data->frame_id,
		trigger_data->trigger);
#endif
...
#if defined(CAMERA_FRS_DRAM_TEST_DBG_LOG)
	CAM_DBG_FRS(CAM_CRM, "link %x req %lld, sync_mode %d",
		sched_req->link_hdl, sched_req->req_id, sched_req->sync_mode);
#endif
