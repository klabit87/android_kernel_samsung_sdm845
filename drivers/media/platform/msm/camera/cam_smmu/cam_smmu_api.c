...
#define CONFIG_CAMERA_MEM 1
#if defined(CONFIG_CAMERA_MEM)
	...
	int ion_fd)
{
	...
	CAM_INFO(CAM_SMMU, "Error: Cannot find fd %d by index %d",
		ion_fd, idx);
	return NULL;
}
	...
	struct cam_dma_buff_info **mapping_info)
{
	...
	if (region_id == CAM_SMMU_REGION_SHARED) {
		...
		if (rc < 0) {
			...
			CAM_ERR(CAM_SMMU, "cam_smmu_alloc_iova : retry (%d)", retry_cnt);
		...
		if (size < 0) {
			...
			CAM_ERR(CAM_SMMU, "iommu_map_sg : retry (%d)", retry_cnt);
		...
		if (rc != table->nents) {
			...
			CAM_ERR(CAM_SMMU, "msm_dma_map_sg_lazy : retry (%d)", retry_cnt);
