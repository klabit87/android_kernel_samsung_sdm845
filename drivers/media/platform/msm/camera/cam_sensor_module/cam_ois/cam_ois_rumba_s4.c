...
int32_t cam_ois_read_user_data_section(struct cam_ois_ctrl_t *o_ctrl, uint16_t addr, int size, uint8_t *user_data)
{
	...
	rc = cam_ois_i2c_byte_write(o_ctrl, 0x000F, 0x12);	// DLFSSIZE_W Register(0x000F) : Size = 4byte * Value
	...
	rc = camera_io_dev_read_seq(&o_ctrl->io_master_info,
		0x0100, read_data,
		CAMERA_SENSOR_I2C_TYPE_WORD, 72);
	if (rc < 0)
		goto ERROR;
	...
	rc = cam_ois_i2c_byte_write(o_ctrl, 0x000F, 0x0E);	// DLFSSIZE_W Register(0x000F) : Size = 4byte * Value
	reg_setting.reg_addr = 0x0010;
	reg_setting.reg_data = 0x0004;
	rc |= cam_qup_i2c_write(&o_ctrl->io_master_info, &reg_setting,
		CAMERA_SENSOR_I2C_TYPE_WORD, CAMERA_SENSOR_I2C_TYPE_WORD); // Data Write Start Address Offset : 0x0400
	rc |= cam_ois_i2c_byte_write(o_ctrl, 0x000E, 0x04); // DFLSCMD Register(0x000E) = READ
	if (rc < 0)
		goto ERROR;
	...
	rc = camera_io_dev_read_seq(&o_ctrl->io_master_info,
		0x0100, shift_data,
		CAMERA_SENSOR_I2C_TYPE_WORD, 56);
	if (rc < 0)
		goto ERROR;
	...
	bool is_force_update)
{
	...
	cam_ois_i2c_byte_write(o_ctrl, 0x0C, 0x75);
	msleep(55);
	...
	struct cam_ois_sinewave_t *sinewave, int *result, int num_of_module)
{
	...
	ret |= cam_ois_i2c_byte_write(o_ctrl, 0x0055, 0x34); /* amplitude level for measurement. */
