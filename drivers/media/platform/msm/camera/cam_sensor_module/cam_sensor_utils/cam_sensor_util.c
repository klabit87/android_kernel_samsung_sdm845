		...
		uint32_t size)
{
	...
	if (size == 0) {
		CAM_ERR(CAM_SENSOR, "cam_sensor_get_i2c_ptr: size is NULL");
		return NULL;
	}
	...
	if (tmp != NULL) {
		memset(tmp, 0, sizeof(struct i2c_settings_list));
	...
	if (tmp->i2c_settings.reg_setting == NULL) {
		list_del(&(tmp->list));
		vfree(tmp);
		CAM_ERR(CAM_SENSOR, "cam_sensor_get_i2c_ptr: %x", size);
		return NULL;
	}
	memset(tmp->i2c_settings.reg_setting, 0,
	...
	uint16_t power_setting_size)
{
	...
	for (i = 0; i < power_setting_size; i++) {
		...
		case SENSOR_CUSTOM_REG8:
			for (j = 0; j < num_vreg; j++) {

				if (!strcmp(soc_info->rgltr_name[j],
					"cam_v_custom8")) {
					CAM_DBG(CAM_SENSOR,
						"i:%d j:%d cam_vcustom8", i, j);
					power_setting[i].seq_val = j;
		...
		case SENSOR_CUSTOM_REG9:
			for (j = 0; j < num_vreg; j++) {

				if (!strcmp(soc_info->rgltr_name[j],
					"cam_v_custom9")) {
					CAM_DBG(CAM_SENSOR,
						"i:%d j:%d cam_vcustom9", i, j);
					power_setting[i].seq_val = j;
		...
		case SENSOR_CUSTOM_REG10:
			for (j = 0; j < num_vreg; j++) {

				if (!strcmp(soc_info->rgltr_name[j],
					"cam_v_custom10")) {
					CAM_DBG(CAM_SENSOR,
						"i:%d j:%d cam_vcustom10", i, j);
					power_setting[i].seq_val = j;
		...
		struct cam_hw_soc_info *soc_info)
{
	...
	CAM_INFO(CAM_SENSOR, "power setting size: %d", ctrl->power_setting_size);
	...
	for (index = 0; index < ctrl->power_setting_size; index++) {
		...
		case SENSOR_CUSTOM_REG10:
			...
			if (power_setting->seq_val < num_vreg) {
				...
				CAM_INFO(CAM_SENSOR, "Enable Regulator %s", soc_info->rgltr_name[vreg_idx]);
	...
	for (index--; index >= 0; index--) {
		...
		case SENSOR_CUSTOM_REG10:
			if (power_setting->seq_val < num_vreg) {
				...
				if (soc_info->rgltr[vreg_idx] != NULL) {
...
#if defined(CONFIG_SENSOR_RETENTION)
			if (pd->seq_type == SENSOR_CUSTOM_REG10 &&
				retention == 1) {
				CAM_DBG(CAM_SENSOR, "[RET_DBG] sensor_retention_mode: %d",
					sensor_retention_mode);
				if (sensor_retention_mode == RETENTION_READY_TO_ON) {
					CAM_INFO(CAM_SENSOR, "[RET_DBG] Skip sensor retention pin low. Retention mode ON");
