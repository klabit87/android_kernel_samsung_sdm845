...
#if 1 //APERTURE_THREAD
#include <linux/wait.h>
#include <linux/freezer.h>
#include <linux/slab.h>
#endif

#define APERTURE_THREAD
...
#define APERTURE_DRIVER_I2C "i2c_aperture"
...
enum cam_aperture_mode {
	CAM_APERTURE_2_4,
	CAM_APERTURE_1_5,
};

#if 1 //APERTURE_THREAD
enum cam_aperture_thread_msg_type {
	CAM_APERTURE_THREAD_MSG_INIT,
	CAM_APERTURE_THREAD_MSG_MAX
};

struct cam_aperture_thread_msg_t {
	struct list_head list;
	int msg_type;
	uint16_t aperture_mode;
...
struct cam_aperture_ctrl_t {
	...
	char device_name[20];
	bool is_initialized;
	enum cam_aperture_mode cam_aper_mode;
#if 1 //APERTURE_THREAD
	struct task_struct *aperture_thread;
	bool is_thread_started;
	struct cam_aperture_thread_msg_t list_head_thread;
	struct mutex aperture_thread_mutex;
