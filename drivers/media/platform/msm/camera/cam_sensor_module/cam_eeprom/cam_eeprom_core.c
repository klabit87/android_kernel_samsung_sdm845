...
static int cam_eeprom_update_module_info(struct cam_eeprom_ctrl_t *e_ctrl)
{
	...
	if (e_ctrl->soc_info.index == 1) {
		...
		if (strcmp(front_phone_fw_ver, front_fw_ver) >= 0) {
			CAM_ERR(CAM_EEPROM, "Load from front phone");
			strcpy(front_load_fw_ver, front_phone_fw_ver);
			loadfrom = 'P';
		} else {
			CAM_ERR(CAM_EEPROM, "Load from front EEPROM");
			strcpy(front_load_fw_ver, front_fw_ver);
			loadfrom = 'E';
		}

		for(i = 0; i < FROM_MODULE_FW_INFO_SIZE; i ++) {
			if(!isalnum(e_ctrl->cal_data.mapdata[FRONT_CAL_VERSION_ADDR + i]))
				cal_ver[i] = ' ';
			else
				cal_ver[i] = e_ctrl->cal_data.mapdata[FRONT_CAL_VERSION_ADDR + i];

			if(!isalnum(front_phone_fw_ver[i]))
		...
		ideal_ver[9] = front_fw_ver[9];
		ideal_ver[10] = front_fw_ver[10];
		...
		snprintf(front_module_info, SYSFS_MODULE_INFO_SIZE, "SSCAL %c%s%04X%04XR%02dM%cD%02XD%02XS%02XS%02X/%s%04X%04XR%02d",
			...
			board_rev, map_ver, dll_ver[0], dll_ver[1], sensor_ver[0], sensor_ver[1],
		...
		if (strcmp(rear_phone_fw_ver, rear_fw_ver) >= 0) {
			CAM_ERR(CAM_EEPROM, "Load from phone");
			strcpy(rear_load_fw_ver, rear_phone_fw_ver);
			loadfrom = 'P';
		} else {
			CAM_ERR(CAM_EEPROM, "Load from EEPROM");
			strcpy(rear_load_fw_ver, rear_fw_ver);
			loadfrom = 'E';
		}

		for(i = 0; i < FROM_MODULE_FW_INFO_SIZE; i ++) {
			if(!isalnum(e_ctrl->cal_data.mapdata[REAR_CAL_VERSION_ADDR + i]))
				cal_ver[i] = ' ';
			else
				cal_ver[i] = e_ctrl->cal_data.mapdata[REAR_CAL_VERSION_ADDR + i];

			if(!isalnum(rear_phone_fw_ver[i]))
				rear_phone_fw_ver[i] = ' ';
		}
