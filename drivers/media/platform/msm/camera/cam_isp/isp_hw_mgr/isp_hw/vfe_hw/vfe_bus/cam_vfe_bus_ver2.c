...
static int cam_vfe_bus_stop_comp_grp(struct cam_isp_resource_node *comp_grp)
{
	...
	/* Disable all register access, rely on global reset */
	#if 0
	cam_io_w_mb(rsrc_data->composite_mask, common_data->mem_base +
		rsrc_data->hw_regs->comp_mask);
	if (rsrc_data->comp_grp_type >= CAM_VFE_BUS_VER2_COMP_GRP_DUAL_0 &&
	...
	struct cam_irq_th_payload *th_payload)
{
    ...
    cam_irq_controller_disable_irq(bus_priv->common_data.bus_irq_controller, bus_priv->error_irq_handle);
	...
	uint32_t arg_size)
{
	...
	for (i = 0, j = 0; i < vfe_out_data->num_wm; i++) {
		...
		if (wm_data->en_ubwc) {
			...
			CAM_DBG(CAM_ISP, "WM %d frame_inc %d: height: %d, stride %d, meta_size: %d",
				wm_data->index, frame_inc, io_cfg->planes[i].slice_height,
				io_cfg->planes[i].plane_stride, io_cfg->planes[i].meta_size);
		}
		else {
