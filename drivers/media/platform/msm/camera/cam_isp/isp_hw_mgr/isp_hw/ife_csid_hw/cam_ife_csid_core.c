	...
	struct cam_isp_resource_node    *res)
{
	...
	/* Disable all register access on deint, reply on global reset */
	/*
	val = cam_io_r_mb(soc_info->reg_map[0].mem_base +
		csid_reg->ipp_reg->csid_ipp_cfg0_addr);
	val &= ~(1 << csid_reg->cmn_reg->path_en_shift_val);
	...
	enum cam_ife_csid_halt_cmd       stop_cmd)
{
	...
	cam_io_w_mb(0, soc_info->reg_map[0].mem_base +
		csid_reg->ipp_reg->csid_ipp_irq_mask_addr);
	/* Disable all the register access in stop. reply on global reset */
	/*
	if (path_data->sync_mode == CAM_ISP_HW_SYNC_MASTER) {
		val = cam_io_r_mb(soc_info->reg_map[0].mem_base +
			csid_reg->ipp_reg->csid_ipp_ctrl_addr);
	...
	struct cam_isp_resource_node    *res)
{
	...
	/* Disable all register access on deinit, reply on global reset */
	/*
	val = cam_io_r_mb(soc_info->reg_map[0].mem_base +
		csid_reg->rdi_reg[id]->csid_rdi_cfg0_addr);
	val &= ~(1 << csid_reg->cmn_reg->path_en_shift_val);
	...
	enum cam_ife_csid_halt_cmd                stop_cmd)
{
	...
	cam_io_w_mb(0, soc_info->reg_map[0].mem_base +
		csid_reg->rdi_reg[id]->csid_rdi_irq_mask_addr);
	/*Halt the RDI path */
	/* Dsable register access on Halt, rely on global reset only */
	/*
	cam_io_w_mb(stop_cmd, soc_info->reg_map[0].mem_base +
			csid_reg->rdi_reg[id]->csid_rdi_ctrl_addr);
	*/
	...
	void *deinit_args, uint32_t arg_size)
{
	...
	case CAM_ISP_RESOURCE_CID:
		...
		break;
	case CAM_ISP_RESOURCE_PIX_PATH:
		CAM_INFO(CAM_ISP, "De-Init Pix Path: %d\n", res->res_id);
		if (res->res_id == CAM_IFE_PIX_PATH_RES_IPP) {
	...
	/* Disable CSID HW */
	CAM_INFO(CAM_ISP, "Disabling CSID Hw\n");
	cam_ife_csid_disable_hw(csid_hw);
	...
	void *stop_args, uint32_t arg_size)
{
	...
	for (i = 0; i < csid_stop->num_res; i++) {
		...
		case CAM_ISP_RESOURCE_CID:
			...
			break;
		case CAM_ISP_RESOURCE_PIX_PATH:
			CAM_INFO(CAM_ISP, "%s: Stop Pix Path: %d\n", __func__, res->res_id);
			if (res->res_id == CAM_IFE_PIX_PATH_RES_IPP)
	...
	for (i = 0; i < csid_stop->num_res; i++) {
		res = csid_stop->node_res[i];
		/* No need to wait for halt, reply on global reset */
		/*
		if (res->res_type == CAM_ISP_RESOURCE_PIX_PATH &&
...
irqreturn_t cam_ife_csid_irq(int irq_num, void *data)
{
	...
	CAM_ERR_RATE_LIMIT(CAM_ISP, "irq_status_rx = 0x%x after clear: 0x%x Mask: 0x%x", irq_status_rx,
	...
	if (irq_status_rx & CSID_CSI2_RX_ERROR_UNBOUNDED_FRAME) {
		CAM_ERR_RATE_LIMIT(CAM_ISP, "%s: irq_status_rx for Unbounded Frame: %d After: %d\n", __func__, irq_status_rx,
			cam_io_r_mb(soc_info->reg_map[0].mem_base +
			csid_reg->csi2_reg->csid_csi2_rx_irq_status_addr));
