...
static struct cam_isp_add_io_buffers_info
	add_io_buffers_info_buf[ADD_IO_BUFS_INFO_MAX_SIZE];
static atomic64_t info_buf_now;
	...
	for (i = 0; i < prepare->packet->num_io_configs; i++) {
		...
		for (j = 0; j < CAM_ISP_HW_SPLIT_MAX; j++) {
			...
			for (plane_id = 0; plane_id < CAM_PACKET_MAX_PLANES;
                ...
                add_io_buffers_info_buf[iter].plane_id = plane_id;
				add_io_buffers_info_buf[iter].io_addr = io_addr[plane_id];
			    add_io_buffers_info_buf[iter].mem_hdl = io_cfg[i].mem_handle[plane_id];
				add_io_buffers_info_buf[iter].mmu_hdl = mmu_hdl;
				add_io_buffers_info_buf[iter].size = size;

				iter = atomic64_add_return(1, &info_buf_now) %
