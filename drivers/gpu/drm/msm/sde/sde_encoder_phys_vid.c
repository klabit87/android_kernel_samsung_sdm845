		...
		bool enable)
{
	...
	if (ret) {
		SDE_ERROR_VIDENC(vid_enc,
				"control vblank irq registration error %d, enable %d\n",
				ret, enable);
		...
		struct sde_encoder_phys *phys_enc, bool notify)
{
	...
	/*
	 * If IRQ is disabled and Commit came, then signal fences to avoid
	 * fence timeouts.
	 */
	irq = &phys_enc->irq[INTR_IDX_VSYNC];
	flag = irq && (irq->irq_idx < 0)
		&& atomic_read(&phys_enc->pending_kickoff_cnt);

	if (ret == -ETIMEDOUT || flag)
	...
	SDE_EVT32(DRMID(phys_enc->parent), event, notify, ret, flag,
