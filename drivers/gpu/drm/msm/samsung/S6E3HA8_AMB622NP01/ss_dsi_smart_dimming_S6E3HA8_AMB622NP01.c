...
#include "ss_dsi_smart_dimming_S6E3HA8_AMB622NP01.h"
...
static void print_hbm_lux_table(struct SMART_DIM *psmart, char *type)
{
	...
	if (psmart->panel_revision < 'G') {
		...
		hbm_interpolation_candela_table = hbm_interpolation_candela_table_revG;
		hbm_steps = HBM_INTERPOLATION_STEP_revG;
	}
...
static int get_base_luminance(struct SMART_DIM *pSmart)
{
	...
	case 'A' ... 'F':
		...
		break;
	case 'G':
		memcpy(base_luminance, base_luminance_star_revG, sizeof(base_luminance_star_revG));
		break;
	default:
		memcpy(base_luminance, base_luminance_star_revG, sizeof(base_luminance_star_revG));
		break;
...
static int get_gradation_offset(int table_index, int index, int panel_revision)
{
	...
	case 'A' ... 'F':
		...
		break;
	case 'G':
		ret = gradation_offset_star_revG[table_index][index];
		break;
	default:
		ret = gradation_offset_star_revG[table_index][index];
		break;
...
static int get_rgb_offset(int table_index, int index, int panel_revision)
{
	...
	case 'A' ... 'F':
		...
		break;
	case 'G':
		ret = rgb_offset_star_revG[table_index][index];
		break;
	default:
		ret = rgb_offset_star_revG[table_index][index];
		break;
...
struct smartdim_conf *smart_get_conf_S6E3HA8_AMB622NP01(void)
{
...
struct smartdim_conf *smart_get_conf_S6E3HA8_AMB622NP01_hmt(void)
{
