...
static struct dsi_panel_cmd_set *ss_acl_on(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	*level_key = LEVEL1_KEY;
	...
	/*
		ACL FRAME AVG UPDATE (B4h 2nd para)
		1. gallery : 16 frame
		2. otheres : 32 frame
	*/
	if (vdd->mdnie_tune_state_dsi->mdnie_app == GALLERY_APP)
		pcmds->cmds[0].msg.tx_buf[2] = 0x40;
	else
		pcmds->cmds[0].msg.tx_buf[2] = 0x50;

	LCD_INFO("gradual_acl: %d, acl fps: 0x%x\n",
			vdd->gradual_acl_val, pcmds->cmds[0].msg.tx_buf[2]);
...
{
	static struct mdnie_lite_tune_data *mdnie_data = NULL;

	if (vdd->support_hall_ic && mdnie_data) {
