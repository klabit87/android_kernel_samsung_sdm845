...
#include "ss_dsi_smart_dimming_S6E3HA6_AMS622MR01.h"
...
static int get_base_luminance(struct SMART_DIM *pSmart)
{
	...
	case 'A' ... 'H':
		memcpy(base_luminance, base_luminance_dream_revA, sizeof(base_luminance_dream_revA));
		break;
	default:
		memcpy(base_luminance, base_luminance_dream_revA, sizeof(base_luminance_dream_revA));
		break;
...
static int get_gradation_offset(int table_index, int index, int panel_revision)
{
	...
	switch (panel_revision) {
	case 'A' ... 'H':
		ret = gradation_offset_dream_revA[table_index][index];
		break;
	case 'S':
		ret = gradation_offset_dream_revS[table_index][index];
		break;
	default:
		ret = gradation_offset_dream_revA[table_index][index];
		break;
...
static int get_rgb_offset(int table_index, int index, int panel_revision)
{
	...
	switch (panel_revision) {
	case 'A' ... 'H':
		ret = rgb_offset_dream_revA[table_index][index];
		break;
	case 'S':
		ret = rgb_offset_dream_revS[table_index][index];
		break;
	default:
		ret = rgb_offset_dream_revA[table_index][index];
		break;
...
static int get_base_luminance_hmt(int brightness_level, int panel_revision)
{
	...
	case 'A' ... 'H':
		...
		break;
	case 'S':
		memcpy(base_luminance, base_luminance_reverse_hmt_single_revS, sizeof(base_luminance_reverse_hmt_single_revS));
		break;
	default:
		memcpy(base_luminance, base_luminance_reverse_hmt_single_revA, sizeof(base_luminance_reverse_hmt_single_revA));
		break;
...
static int get_gradation_offset_hmt(int table_index, int index, int panel_revision)
{
	...
	case 'S':
		return gradation_offset_reverse_hmt_single_revS[table_index][index];
	default:
		return gradation_offset_reverse_hmt_single_revA[table_index][index];
	}
}
...
static int get_rgb_offset_hmt(int table_index, int index, int panel_revision)
{
	...
	case 'S':
		return rgb_offset_reverse_hmt_single_revS[table_index][index];
	default:
		return rgb_offset_reverse_hmt_single_revA[table_index][index];
	}
}
