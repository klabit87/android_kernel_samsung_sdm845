...
#include "ss_dsi_smart_dimming_S6E3HA8_AMB577PX01.h"
...
static void print_hbm_lux_table(struct SMART_DIM *psmart, char *type)
{
	...
	if (psmart->panel_revision < 'H') {
		hbm_interpolation_candela_table = hbm_interpolation_candela_table_revA;
		hbm_steps = HBM_INTERPOLATION_STEP;
	} else {
		hbm_interpolation_candela_table = hbm_interpolation_candela_table_revH;
		hbm_steps = HBM_INTERPOLATION_STEP_revH;
	}
...
int hbm_interpolation_gamma[HBM_INTERPOLATION_STEP_revH][PACKED_GAMM_SET_CNT];
