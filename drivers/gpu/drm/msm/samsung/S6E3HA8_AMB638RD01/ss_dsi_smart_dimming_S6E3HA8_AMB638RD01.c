...
#include "ss_dsi_smart_dimming_S6E3HA8_AMB638RD01.h"
...
static void TP_gamma_code_calc(struct SMART_DIM *pSmart, int *M_GRAY, int *str)
{
	...
	/* V0/VT */
	str[cnt++] = (center_gamma[V0][R] & 0xF) << 4 | (center_gamma[V0][G] & 0xF);
	str[cnt++] = (center_gamma[V0][B] & 0xF) << 4 | (center_gamma[VT][R] & 0xF);
	str[cnt++] = (center_gamma[VT][G] & 0xF) << 4 | (center_gamma[VT][B] & 0xF);
}
...
struct smartdim_conf *smart_get_conf_S6E3HA8_AMB638RD01(void)
{
...
struct smartdim_conf *smart_get_conf_S6E3HA8_AMB638RD01_hmt(void)
{
