...
int dsi_panel_prepare(struct dsi_panel *panel)
{
	...
	if (vdd->support_hall_ic) {
		gpio_set_value(vdd->select_panel_gpio, vdd->hall_ic_status_unhandled);
		vdd->hall_ic_status = vdd->hall_ic_status_unhandled;
		pr_info("hall_ic_status %s\n", (vdd->hall_ic_status == HALL_IC_OPEN) ? "OPEN" : "CLOSE");
	}
