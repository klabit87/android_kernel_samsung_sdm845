...
static int dsi_display_cmd_engine_enable(struct dsi_display *display)
{
	...
	SDE_EVT32(display->cmd_engine_refcount);
...
static int dsi_display_cmd_engine_disable(struct dsi_display *display)
{
	...
	for (i = 0; i < display->ctrl_count; i++) {
		...
		rc = dsi_ctrl_set_cmd_engine_state(ctrl->ctrl,
						   DSI_CTRL_ENGINE_OFF);
		if (rc)
			pr_err("[%s] failed to disable cmd engine, rc=%d\n",
			       display->name, rc);
	}
...
static int dsi_display_ctrl_update(struct dsi_display *display)
{
	...
	for (i = 0 ; i < display->ctrl_count; i++) {
		ctrl = &display->ctrl[i];
		rc = dsi_ctrl_host_timing_update(ctrl->ctrl, display->is_cont_splash_enabled);
		if (rc) {
...
static void dsi_display_aspace_cb_locked(void *cb_data, bool is_detach)
{
	...
	for (cnt = 0; cnt < display->ctrl_count; cnt++) {
		...
		pr_info("display_ctrl->ctrl->secure_mode = %d, cnt = %d\n", display_ctrl->ctrl->secure_mode, cnt);
	}
