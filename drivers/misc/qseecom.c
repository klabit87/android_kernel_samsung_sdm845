...
static void __qseecom_listener_abort_all(void) 
{ 
    struct qseecom_registered_listener_list *entry = NULL; 
    unsigned long flags; 
    ...
    list_for_each_entry(entry,&qseecom.registered_listener_list_head, list) { 
      if (entry->listener_in_use == true) { 
        pr_debug("abort listener %d\n", entry->svc.listener_id); 
        entry->abort = 1; 
      } 
    } 
...
static int __qseecom_listener_has_sent_rsp(struct qseecom_dev_handle *data, struct qseecom_registered_listener_list *ptr_svc) 
{
	int ret;
					...
					struct qseecom_command_scm_resp *resp)
{
	...
	while (resp->result == QSEOS_RESULT_INCOMPLETE) {
		...
		pr_err("wake up tasks waiting on listener %d\n", lstnr);
		if (ret) {
			pr_err("scm_call() failed with err: %d (app_id = %d)\n",
				ret, data->client.app_id);
			if ((lstnr == RPMB_SERVICE) || (lstnr == SSD_SERVICE))
				...
				struct qseecom_dev_handle *data)
{
	...
	do {
		...
		do {
			pr_err("1. lsntr %d in_use = %d, qseecom.app_block_ref_cnt = %d\n",
					resp->data, list_ptr->listener_in_use, qseecom.app_block_ref_cnt);
			qseecom.app_block_ref_cnt++;
			ptr_app->app_blocked = true;
			mutex_unlock(&app_access_lock);
			pr_err("2. lsntr %d in_use = %d, qseecom.app_block_ref_cnt = %d\n",
					resp->data, list_ptr->listener_in_use, qseecom.app_block_ref_cnt);
			wait_event_freezable(
				list_ptr->listener_block_app_wq,
				!list_ptr->listener_in_use);
			pr_err("3. lsntr %d in_use = %d, qseecom.app_block_ref_cnt = %d\n",
					resp->data, list_ptr->listener_in_use, qseecom.app_block_ref_cnt);
			mutex_lock(&app_access_lock);
			pr_err("4. lsntr %d in_use = %d, qseecom.app_block_ref_cnt = %d\n",
					resp->data, list_ptr->listener_in_use, qseecom.app_block_ref_cnt);
			...
			struct qseecom_registered_app_list *ptr_app)
{
	...
	if (qseecom.qsee_reentrancy_support) {
		while (ptr_app->app_blocked || qseecom.app_block_ref_cnt > 1) {
			...
			sigfillset(&new_sigset);
			sigprocmask(SIG_SETMASK, &new_sigset, &old_sigset);
			pr_err("1. app_blocked = %d, app_block_ref_cnt = %d\n",
				ptr_app->app_blocked, qseecom.app_block_ref_cnt);
			mutex_unlock(&app_access_lock);
			pr_err("2. app_blocked = %d, app_block_ref_cnt = %d\n",
				ptr_app->app_blocked, qseecom.app_block_ref_cnt);
			do {
				if (!wait_event_freezable(qseecom.app_block_wq,
					(!ptr_app->app_blocked &&
			...
			mutex_lock(&app_access_lock);
			pr_err("3. app_blocked = %d, app_block_ref_cnt = %d\n",
				ptr_app->app_blocked, qseecom.app_block_ref_cnt);
			sigprocmask(SIG_SETMASK, &old_sigset, NULL);
		}
	}
				...
				struct qseecom_dev_handle *data)
{
	...
	case QSEOS_RESULT_INCOMPLETE:
		qseecom.app_block_ref_cnt++;
		ptr_app->app_blocked = true;
		pr_err("1. begin req lsntr %d , qseecom.app_block_ref_cnt = %d, app %d, blocked %d\n",
			resp->data, qseecom.app_block_ref_cnt,
			data->client.app_id, ptr_app->app_blocked);
		ret = __qseecom_reentrancy_process_incomplete_cmd(data, resp);
		ptr_app->app_blocked = false;
		qseecom.app_block_ref_cnt--;
		pr_err("2. end req lsntr %d , qseecom.app_block_ref_cnt = %d, app %d, blocked %d\n",
			resp->data, qseecom.app_block_ref_cnt,
			data->client.app_id, ptr_app->app_blocked);
...
int qseecom_process_listener_from_smcinvoke(struct scm_desc *desc)
{
	...
	pr_err("cmd %d, session %d, listener %d\n", (int)desc->ret[0], (int)desc->ret[1], (int)desc->ret[2]);
		...
		if (qseecom.qsee_version >= (QSEE_VERSION_02) &&
			...
			if (resource) {
				if (qseecom.qsee_version < QSEE_VERSION_40) {
					...
					sec_debug_secure_app_addr_size(req.addr,
								req.size);
					...
					sec_debug_secure_app_addr_size(
