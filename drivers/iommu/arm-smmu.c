...
static void arm_smmu_rtb_log(struct arm_smmu_device *smmu, u32 val, void __iomem *addr);
					...
					size_t size, gfp_t gfp_mask)
{
	...
	if (!arm_smmu_has_secure_vmid(smmu_domain)) {
		struct page *pg;
	        /* size is expected to be 4K with current pagetable configuration */
		if (size == PAGE_SIZE) {
		        pg = list_first_entry_or_null(
                    &smmu_domain->nonsecure_pool,struct page, lru);
		    if (pg) {
			    pr_debug("got pool page\n");
...
static void arm_smmu_write_smr(struct arm_smmu_device *smmu, int idx)
{
	...
	addr = ARM_SMMU_GR0(smmu) + ARM_SMMU_GR0_SMR(idx);
	writel_relaxed(reg, addr);
	arm_smmu_rtb_log(smmu, reg, addr);
}
					...
					size_t size, struct list_head *pool)
{
	...
	pr_debug("preallocated 0x%x pages for size 0x%zx\n", i, size);
}
...
static void arm_smmu_rtb_log(struct arm_smmu_device *smmu, u32 val, void __iomem *_addr)
{
	u64 addr = (u64)_addr;
	unsigned long flags;
	struct arm_smmu_rtb *log;

	spin_lock_irqsave(&smmu->rtb_lock, flags);
	log = smmu->rtb_log + smmu->rtb_index;
	log->time_ns = sched_clock();
	log->addr = addr;
	log->val = val;
	/* Hypervisor may modify what we write, if they think it is incorrect */
	log->val2 = readl_relaxed(_addr);
	log->same = !!(log->val == log->val2);

	smmu->rtb_index++;
	if (smmu->rtb_index >= NR_RTB_ENTRIES)
		smmu->rtb_index = 0;
	spin_unlock_irqrestore(&smmu->rtb_lock, flags);
}

static int arm_smmu_rtb_log_init(struct arm_smmu_device *smmu)
{
	struct arm_smmu_rtb *ptr;

	ptr = vmalloc(sizeof(*ptr) * NR_RTB_ENTRIES);
	if (!ptr)
		return -ENOMEM;

	smmu->rtb_log = ptr;
	smmu->rtb_index = 0;
	spin_lock_init(&smmu->rtb_lock);

	return 0;
}

static void arm_smmu_rtb_log_free(struct arm_smmu_device *smmu)
{
	if (smmu->rtb_log)
		vfree(smmu->rtb_log);
}
...
static int arm_smmu_device_dt_probe(struct platform_device *pdev)
{
	...
	err = arm_smmu_rtb_log_init(smmu);
	if (err)
		goto out_exit_power_resources;
