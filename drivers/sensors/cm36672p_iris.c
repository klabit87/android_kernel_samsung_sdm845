...
#define MODULE_NAME     "iris_proximity_sensor"
...
#define DEFAULT_HI_THD  0x000a
#define DEFAULT_LOW_THD 0x0002
#define CANCEL_HI_THD   0x0008
#define CANCEL_LOW_THD  0x0005

#define DEFAULT_CONF1   0x03ac /* PS_INT = (1:1), PS_PERS = (1:0) */
#define DEFAULT_CONF3   0x4210 /* PS_MS = 1, PS_SMART_PERS = 0 */
#define DEFAULT_TRIM    0x001b
...
struct cm36672p_data {
	...
	struct hrtimer iris_prox_timer;
	struct workqueue_struct *iris_prox_wq;
	struct work_struct work_iris_prox;
...
static int proximity_vio_onoff(struct device *dev, bool onoff)
{
	...
	if (!data->vio) {
		...
		data->vio = devm_regulator_get(dev, "cm36672p_iris,vio");
		if (IS_ERR(data->vio)) {
			SENSOR_ERR("cannot get vio\n");
			data->vio = NULL;
			return -ENOMEM;
		}
		if (!regulator_get_voltage(data->vio))
			regulator_set_voltage(data->vio, 1800000, 1800000);
	}

	if (onoff) {
		if (regulator_is_enabled(data->vio)) {
			SENSOR_INFO("vio already enabled\n");
		} else {
...
static int proximity_vdd_onoff(struct device *dev, bool onoff)
{
	...
	if (!data->vdd) {
		...
		data->vdd = devm_regulator_get(dev, "cm36672p_iris,vdd");
		if (IS_ERR(data->vdd)) {
		...
		if (!regulator_get_voltage(data->vdd))
			regulator_set_voltage(data->vdd, 3300000, 3300000);
	}
...
static int proximity_vled_onoff(struct device *dev, bool onoff)
{
	...
	if (!data->vled) {
		...
		data->vled = devm_regulator_get(dev, "cm36672p_iris,vled");
		if (IS_ERR(data->vled)) {
	...
	if (onoff) {
		if (regulator_is_enabled(data->vled)) {
			SENSOR_INFO("vled already enabled\n");
		} else {
...
static int proximity_store_cancelation(struct device *dev, bool do_calib)
{
	...
	if (do_calib) {
		...
		} else if (ps_data <= data->pdata->offset_range_hi) {
			/* CANCELATION */
			ps_reg_init_setting[PS_CANCEL][CMD] =
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (new_value && !pre_enable) {
		int i, ret;
		if (!data->pdata->vio_ldo)
			proximity_vio_onoff(dev, ON);
		...
		hrtimer_start(&data->iris_prox_timer,
			data->poll_delay, HRTIMER_MODE_REL);
		data->is_send_event = true;
	} else if (!new_value && pre_enable) {
		disable_irq_wake(data->irq);
		disable_irq(data->irq);
		if (data->is_send_event) {
			hrtimer_cancel(&data->iris_prox_timer);
			cancel_work_sync(&data->work_iris_prox);
		}
		/* disable settings */
		cm36672p_i2c_write_word(data, REG_PS_CONF1, 0x0001);
		atomic_set(&data->enable, OFF);
		if (!data->pdata->vio_ldo)
			proximity_vio_onoff(dev, OFF);
		if (!data->pdata->regulator_divided)
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	SENSOR_INFO("new delay = %lldns, old delay = %lldns\n",
		new_delay, ktime_to_ns(data->poll_delay));
...
irqreturn_t proximity_irq_thread_fn(int irq, void *user_data)
{
	...
	if (enabled) {
		...
		{
			if (data->is_send_event && !val) {
				SENSOR_INFO("cancelling poll timer\n");
				data->is_send_event = false;
				hrtimer_cancel(&data->iris_prox_timer);
				cancel_work_sync(&data->work_iris_prox);
			}
			/* 0 is close, 1 is far */
...
static void cm36672_work_func_iris_prox(struct work_struct *work)
{
	struct cm36672p_data *data = container_of(work,
		struct cm36672p_data, work_iris_prox);

	input_report_rel(data->proximity_input_dev, REL_MISC, 1);
	input_sync(data->proximity_input_dev);
}

static enum hrtimer_restart cm36672_iris_prox_timer_func(struct hrtimer *timer)
{
	struct cm36672p_data *data = container_of(timer,
		struct cm36672p_data, iris_prox_timer);
	queue_work(data->iris_prox_wq, &data->work_iris_prox);
	hrtimer_forward_now(&data->iris_prox_timer, data->poll_delay);
	...
	struct cm36672p_platform_data *pdata)
{
	...
	pdata->irq = of_get_named_gpio_flags(np, "cm36672p_iris,irq_gpio", 0,
		&flags);
	if (pdata->irq < 0) {
	...
	pdata->vled_ldo = of_get_named_gpio_flags(np, "cm36672p_iris,vled_ldo",
		0, &flags);
	if (pdata->vled_ldo < 0) {
	...
	pdata->vio_ldo = of_get_named_gpio_flags(np, "cm36672p_iris,vio_ldo",
		0, &flags);
	if (pdata->vio_ldo < 0) {
		SENSOR_ERR("Not defined vio_ldo: means to use regulator as vio\n");
		pdata->vio_ldo = 0;
	} else {
		ret = gpio_request(pdata->vio_ldo, "prox_vio_en");
		if (ret < 0)
			SENSOR_ERR("gpio %d request failed (%d)\n",
				pdata->vio_ldo, ret);
		else
			gpio_direction_output(pdata->vio_ldo, 0);
	}

	ret = of_property_read_u32(np, "cm36672p_iris,vdd_always_on",
		&pdata->vdd_always_on);

	ret = of_property_read_u32(np, "cm36672p_iris,regulator_divided",
		&pdata->regulator_divided);

	ret = of_property_read_u32(np, "cm36672p_iris,default_hi_thd",
	...
	ret = of_property_read_u32(np, "cm36672p_iris,default_low_thd",
	...
	ret = of_property_read_u32(np, "cm36672p_iris,cancel_hi_thd",
		&pdata->cancel_hi_thd);
	if (ret < 0) {
	...
	ret = of_property_read_u32(np, "cm36672p_iris,cancel_low_thd",
	...
	ret = of_property_read_u32(np, "cm36672p_iris,offset_range_hi",
	...
	ret = of_property_read_u32(np, "cm36672p_iris,offset_range_low",
	...
	ret = of_property_read_u32(np, "cm36672p_iris,ps_duty", &temp);
	if (ret < 0) {
	...
	ret = of_property_read_u32(np, "cm36672p_iris,ps_pers", &temp);
	if (ret < 0) {
	...
	ret = of_property_read_u32(np, "cm36672p_iris,ps_it", &temp);
	if (ret < 0) {
	...
	ret = of_property_read_u32(np, "cm36672p_iris,led_current", &temp);
	if (ret < 0) {
	...
	ret = of_property_read_u32(np, "cm36672p_iris,ps_smart_pers", &temp);
	if (ret < 0) {
	...
	ret = of_property_read_u32(np, "cm36672p_iris,default_trim",
		&pdata->default_trim);
	if (ret < 0) {
...
static int cm36672p_iris_i2c_probe(struct i2c_client *client,
	const struct i2c_device_id *id)
{
	...
	proximity_vio_onoff(&client->dev, ON);
	...
	input_set_capability(data->proximity_input_dev, EV_REL, REL_MISC);
	...
	ret = sensors_create_symlink(data->proximity_input_dev);
	if (ret < 0) {
	...
	hrtimer_init(&data->iris_prox_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	data->poll_delay = ns_to_ktime(2000 * NSEC_PER_MSEC);/*2 sec*/
	data->iris_prox_timer.function = cm36672_iris_prox_timer_func;

	/* the timer just fires off a work queue request.  we need a thread
	*  to read the i2c (can be slow and blocking).
	*/
	data->iris_prox_wq = create_singlethread_workqueue("cm36672_iris_wq");
	if (!data->iris_prox_wq) {
		ret = -ENOMEM;
		SENSOR_ERR("could not create iris_prox workqueue\n");
		goto err_create_iris_prox_workqueue;
	}
	/* this is the thread function we run on the work queue */
	INIT_WORK(&data->work_iris_prox, cm36672_work_func_iris_prox);
	...
	if (!data->pdata->vio_ldo)
		proximity_vio_onoff(&client->dev, OFF);
	if (!data->pdata->regulator_divided)
...
err_prox_sensor_register:
	destroy_workqueue(data->iris_prox_wq);
err_create_iris_prox_workqueue:
...
err_sysfs_create_group_proximity:
	sensors_remove_symlink(data->proximity_input_dev);
...
static int cm36672p_iris_i2c_remove(struct i2c_client *client)
{
	SENSOR_INFO("\n");
	return 0;
}

static void cm36672p_iris_i2c_shutdown(struct i2c_client *client)
{
...
static struct i2c_driver cm36672p_iris_i2c_driver = {
	.driver = {
		   .name = "cm36672p_iris",
		   .owner = THIS_MODULE,
	...
	.probe = cm36672p_iris_i2c_probe,
	.remove = cm36672p_iris_i2c_remove,
	.shutdown = cm36672p_iris_i2c_shutdown,
	.id_table = cm36672p_device_id,
};

static int __init cm36672p_iris_init(void)
{
	return i2c_add_driver(&cm36672p_iris_i2c_driver);
}

static void __exit cm36672p_iris_exit(void)
{
	i2c_del_driver(&cm36672p_iris_i2c_driver);
}

module_init(cm36672p_iris_init);
module_exit(cm36672p_iris_exit);

MODULE_AUTHOR("Samsung Electronics");
MODULE_DESCRIPTION("Proximity Sensor for IRIS device driver for CM36672P");
MODULE_LICENSE("GPL");
