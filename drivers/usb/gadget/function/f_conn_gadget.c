				...
				struct usb_endpoint_descriptor *out_desc)
{
	...
	if (!ep) {
		pr_err("usb_ep_autoconfig for ep_out failed\n");
		return -ENODEV;
	}
	...
	for (i = 0; i < CONN_GADGET_RX_REQ_MAX; i++) {
		req = conn_gadget_request_new(dev->ep_out,
				(dev->transfer_size)?dev->transfer_size :
					CONN_GADGET_DEFAULT_TRANSFER_SIZE); //use default value
		if (!req)
			goto fail;
...
static int conn_gadget_setup(struct conn_gadget_instance *fi_conn_gadget)
{
	...
	if (!dev) {
		pr_err("alloc conn_gadget_dev F\n");
		return -ENOMEM;
	}
	...
	if (!dev->rd_queue_buf) {
		pr_err("%s: error rd_queue vmalloc\n", __func__);
		ret = -ENOMEM;
		goto err_;
	}
