...
retry:
	...
	msg_maxim("chip : %02X.%02X, FW : %02X.%02X",
			max77705->FW_Revision, max77705->FW_Minor_Revision, fw_bin[4], fw_bin[5]);
	store_ccic_bin_version(&fw_bin[4], &sw_boot);
#ifdef CONFIG_SEC_FACTORY
	if ((max77705->FW_Revision != fw_bin[4]) || (max77705->FW_Minor_Revision != fw_bin[5])) {
#else
	if ((max77705->FW_Revision == 0xff) || (max77705->FW_Revision < fw_bin[4]) ||
		((max77705->FW_Revision == fw_bin[4]) && (max77705->FW_Minor_Revision < fw_bin[5])) ||
		enforce_do) {
#endif
