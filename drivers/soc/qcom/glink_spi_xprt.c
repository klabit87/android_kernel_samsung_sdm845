				 ...
				 uint32_t lcid, uint32_t liid, bool reuse)
{
	...
	GLINK_ERR("%s: Sending rx_done\n", __func__);
	glink_spi_xprt_tx_cmd(einfo, &cmd, sizeof(cmd));
	GLINK_ERR("%s: Sent rx_done\n", __func__);
	srcu_read_unlock(&einfo->use_ref, rcu_id);
}
				...
				uint32_t lcid, size_t size)
{
	...
	pr_info("%s: requesting intent size %zu \n", __func__, size);
	glink_spi_xprt_tx_cmd(einfo, &cmd, sizeof(cmd));

	pr_info("%s:  intent requested for size %zu \n", __func__, size);
	srcu_read_unlock(&einfo->use_ref, rcu_id);
	return 0;
}
		   ...
		   uint32_t lcid, struct glink_core_tx_pkt *pctx)
{
	...
	if (!pctx->size_remaining) {
		GLINK_ERR("%s: size remaining is zero.\n",
			  __func__);
		return 0;
	}
