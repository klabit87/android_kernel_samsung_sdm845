...
void etspi_set_nontz_cpu_min_lock(struct etspi_data *etspi, bool on_off)
{
	int retval = 0;
	...
	if (on_off) {
		u8 retry_cnt = 0;
#ifdef FP_DDR_FREQ_CONTROL
		pr_info("%s FP_CPU_SPEEDUP ON:%d PQ ddr req \n",
#else
		pr_info("%s FP_CPU_SPEEDUP ON:%d\n",
#endif
				__func__, on_off);
		if (etspi->min_cpufreq_limit) {
			if (!atomic_read(&etspi->is_boosting)) {
				pm_qos_add_request(&etspi->pm_qos,
					PM_QOS_CPU_DMA_LATENCY, 0);
#ifdef FP_DDR_FREQ_CONTROL
				msm_bus_scale_client_update_request(bus_hdl, MHZ_1803);
#endif
				do {
					...
					if (retval) {
						...
						atomic_set(&etspi->is_boosting, 1);
				} while (retval && retry_cnt < 3);
			}
			/* Even if boosting is not set by dupulication,
			 * jiffies time should be updated to last
			 * the boosting status for sensor working
			 */
			pre_jiffies = jiffies;
		}
	} else {
		if (etspi->min_cpufreq_limit &&
			atomic_read(&etspi->is_boosting)) {
#ifdef FP_DDR_FREQ_CONTROL
		pr_info("%s FP_CPU_SPEEDUP OFF PQ ddr req\n", __func__);
#else
		pr_info("%s FP_CPU_SPEEDUP OFF\n", __func__);
			...
			if (retval)
				...
				atomic_set(&etspi->is_boosting, 0);
#ifdef FP_DDR_FREQ_CONTROL
...
#ifdef NONTZ_BOOST_CONTROL
		/* cpu minlock control on */
		if (atomic_read(&etspi->is_sensor_ok))
			etspi_set_nontz_cpu_min_lock(etspi, true);
#endif
	} else if (status == 0) {
...
#ifdef NONTZ_BOOST_CONTROL
		/* cpu minlock control off */
		if (atomic_read(&etspi->is_sensor_ok))
			etspi_set_nontz_cpu_min_lock(etspi, false);
#endif
	} else {
...
static long etspi_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case FP_CPU_SPEEDUP:
		...
		if (ioc->len) {
			u8 retry_cnt = 0;
#ifdef FP_DDR_FREQ_CONTROL
			pr_info("%s FP_CPU_SPEEDUP ON:%d, retry: %d PQ ddr req \n",
#else
			...
			if (etspi->min_cpufreq_limit) {
				pm_qos_add_request(&etspi->pm_qos,
					PM_QOS_CPU_DMA_LATENCY, 0);
#ifdef FP_DDR_FREQ_CONTROL
				msm_bus_scale_client_update_request(bus_hdl, MHZ_1296);
#endif
				do {
					retval = set_freq_limit(DVFS_FINGER_ID,
						etspi->min_cpufreq_limit);
...
#ifdef FP_DDR_FREQ_CONTROL
			pr_info("%s FP_CPU_SPEEDUP OFF PQ ddr rm\n", __func__);
#else
			pr_info("%s FP_CPU_SPEEDUP OFF\n", __func__);
#endif
			retval = set_freq_limit(DVFS_FINGER_ID, -1);
...
#ifdef NONTZ_BOOST_CONTROL
	if (atomic_read(&g_data->is_sensor_ok)
		&& atomic_read(&g_data->is_boosting)) {
		/* If boosting time is over 5 sec, it would be off */
		if (jiffies_to_msecs(jiffies - pre_jiffies) > 5000)
			etspi_set_nontz_cpu_min_lock(g_data, false);
	}
	pr_info("%s ldo: %d, sleep: %d, tz: %d, type: %s boost: %d\n",
		__func__,
...
#ifdef NONTZ_BOOST_CONTROL
	atomic_set(&etspi->is_sensor_ok, 1);
#endif
