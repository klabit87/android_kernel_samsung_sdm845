...
static u32 get_bin_checksum(struct sec_ts_data *ts, const u8 *data, size_t size)
{
	...
	input_err(true, &ts->client->dev, "HQJ ver %d, flip_status %d, checksum[0x%x, 0x%x, 0x%x, 0x%x]", 
	...
	if (ts->plat_data->img_version_of_ic[3] >= 9)
		img_checksum = chunk_checksum[0] + ( ts->flip_status_current ? chunk_checksum[2]:chunk_checksum[1] );
	else if ((ts->plat_data->img_version_of_ic[3] < 9) && (ts->plat_data->img_version_of_ic[3] >= 0))
		img_checksum = chunk_checksum[0] + ( ts->flip_status_current ? chunk_checksum[3]:chunk_checksum[2] );
	else
