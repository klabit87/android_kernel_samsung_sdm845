...
struct sec_ts_data {
	...
	struct mutex sec_ts_switching_mutex;
	struct delayed_work switching_work;
	struct notifier_block hall_ic_nb;
	u8 broken_tsp_flag; // 00 is okay, 01 is MAIN broken, 10 is SUB broken, 11 is two broken.
#endif
...
extern int get_lcd_attached_secondary(char *mode);
