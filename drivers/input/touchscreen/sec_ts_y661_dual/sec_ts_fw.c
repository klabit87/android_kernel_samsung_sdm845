...
static int sec_ts_firmware_update(struct sec_ts_data *ts, const u8 *data, size_t size, int bl_update, int restore_cal, int retry)
{
	...
	temp_system_mode = ts->lowpower_status;
	ts->lowpower_status = TO_FLASH_MODE;
	...
	if (!bl_update) {
		...
		if (restore_cal == 1) {
			...
			/* only calibrate with sub TSP, 0=main TSP, 1=sub TSP */
			if(gpio_get_value(ts->plat_data->tsp_sel) == 1)
#endif
...
EXIT:
	ts->lowpower_status = temp_system_mode;
#ifdef CONFIG_DUAL_TSP
	if(ts->change_flip_status){
		input_info(true, &ts->client->dev, "%s: re-try after fw updating, %d\n", __func__, ts->change_flip_status);
		schedule_delayed_work(&ts->switching_work, msecs_to_jiffies(1));
	}
#endif
