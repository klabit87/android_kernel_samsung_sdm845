...
if back light is ON before keypad assemble, it is not good for eyes.
so factory bin echo 2 to brighten in key test of 15, echo 3 to light off the keypad LED.
And other all cases should not light on keypad LED.
*/
	...
	if(data==2){
		...
		dev_err(dev, "%s: factory bin light off the keypad led\n", __func__);
		data=0;
	} else {
		dev_err(dev, "%s: factory ignore control the keypad led(%d)\n", __func__, data);
		return size;
	}
#endif
