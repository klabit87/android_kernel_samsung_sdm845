...
struct max77854_afc_init_data_s {
	struct work_struct muic_afc_init_work;
	struct hv_data *phv;
};
struct max77854_afc_init_data_s afc_init_data;

static bool max77854_muic_check_is_hv_dev(struct hv_data *phv)
{
	bool ret = false;
...
static void max77854_hv_muic_detect_dev(struct hv_data *phv, int irq)
{
	...
	MUIC_INFO_K("%s:%s STATUS1:0x%02x, 2:0x%02x, 3:0x%02x\n", MUIC_DEV_NAME, __func__,
	...
	MUIC_INFO_K("%s:%s HVCONTROL1:0x%02x, 2:0x%02x\n", MUIC_HV_DEV_NAME, __func__,
	...
	MUIC_INFO_K("%s:%s vdnmon:0x%x mpnack:0x%x vbadc:0x%x dpdnvden:0x%x\n",
	...
	for (i = 0; i < ATTACHED_DEV_NUM; i++, tmp_afc_data = tmp_afc_data->next) {
		...
		MUIC_INFO_K("%s:%s checklist match found at i(%d), %s(%d)\n",
...
{
	struct max77854_afc_init_data_s *init_data =
	    container_of(work, struct max77854_afc_init_data_s, muic_afc_init_work);
...
int muic_max77854_afc_set_voltage(int vol)
{
...
int muic_max77854_set_afc(bool enable)
{
	muic_data_t *pmuic = hv_afc.pmuic;
...
static void max77854_afc_check_qc_vb(struct work_struct *work)
{
...
static void max77854_afc_check_mping_miss(struct work_struct *work)
{
...
void max77854_afc_init_detect(struct hv_data *phv)
{
	int ret;
	u8 status3, vdnmon;
...
static void max77854_hv_muic_initialize(struct hv_data *phv)
{
	...
	INIT_DELAYED_WORK(&phv->hv_muic_qc_vb_work, max77854_afc_check_qc_vb);
	INIT_DELAYED_WORK(&phv->hv_muic_mping_miss_wa, max77854_afc_check_mping_miss);
}
 ...
 *
 */
void max77854_afc_initialize(muic_data_t *pmuic, struct hv_data **pphv)
{
	pr_info("%s:%s\n", MUIC_HV_DEV_NAME, __func__);
...
void max77854_afc_clear_hvcontrol(struct hv_data *phv)
{
	struct i2c_client *i2c;
	if (!phv) {
...
void max77854_afc_configure_AFC(struct hv_data *phv)
{
	if (!phv) {
		pr_err("%s:%s: hv is not ready.\n", __func__, MUIC_HV_DEV_NAME);
		return;
...
bool max77854_afc_do_predetach(struct hv_data *phv, int mdev)
{
        bool noti;
...
void max77854_afc_do_detach(struct hv_data *phv)
{
	if (!phv) {
		pr_err("%s:%s: hv is not ready.\n", __func__, MUIC_HV_DEV_NAME);
		return;
