...
BUILD_ROOT_DIR=$(cd -- "$BUILD_KERNEL_DIR/../../.." && pwd)
...
FUNC_BUILD_KERNEL()
{
    ...
    echo '--cmdline "androidboot.hardware=qcom video=vfb:640x400,bpp=32,memsize=3072000 msm_rtb.filter=0x237 ehci-hcd.park=3 lpm_levels.sleep_disabled=1 service_locator.enable=1 swiotlb=2048" --base 0x00000000 --pagesize 4096 --os_version 8.1.0 --os_patch_level 2018-04-01 --ramdisk_offset 0x02000000 --tags_offset 0x01E00000 '   >  $PRODUCT_OUT/mkbootimg_ver_args.txt   
	...
	if  [[ "${BUILD_OPTION_SECOND_BUILD}" != "yes" ]] ; then
