			    ...
			    u32 scrambler_mode)
{
	...
	if ((enc_format != ASM_MEDIA_FMT_NONE) &&
	    (cfg_type == AFE_PARAM_ID_SLIMBUS_CONFIG)) {
		pr_info("%s: AFE_PARAM_ID_SLIMBUS_CONFIG enc_format %4X\n", __func__, enc_format);
		config.port.slim_sch.data_format =
				AFE_SB_DATA_FORMAT_GENERIC_COMPRESSED;
	} else if ((enc_format != ASM_MEDIA_FMT_NONE) && 
		(cfg_type == AFE_PARAM_ID_I2S_CONFIG)) { 
		pr_info("%s: AFE_PARAM_ID_I2S_CONFIG enc_format %4X\n", __func__, enc_format);
