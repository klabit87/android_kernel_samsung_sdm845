...
#include <asoc/maxim_dsm_cal.h>
#include <asoc/maxim_dsm.h>
#include <asoc/maxim_dsm_power.h>
...
static int __max98512_spk_enable(struct max98512_priv *max98512)
{
	...
	digital_gain_l = max98512->digital_gain_left;
...
static int max98512_left_digital_gain_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct max98512_priv *max98512 = snd_soc_codec_get_drvdata(codec);

	ucontrol->value.integer.value[0] = max98512->digital_gain_left;
	msg_maxim("digital_gain_left setting returned %d",
		  (int) ucontrol->value.integer.value[0]);

	return 0;
}

static int max98512_left_digital_gain_put(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct max98512_priv *max98512 = snd_soc_codec_get_drvdata(codec);
	...
	if (sel < (1 << MAX98512_AMP_VOL_WIDTH) - 1) {
		max98512_wrapper_update(max98512, MAX98512L,
					...
					MAX98512_AMP_VOL_MASK,
					sel);
		max98512->digital_gain_left = sel;
	}
...
static const struct snd_kcontrol_new max98512_snd_controls[] = {
	...
	SOC_SINGLE_EXT_TLV("Rcv Digital Gain",
			   ...
			   max98512_digital_tlv),
	SOC_SINGLE_EXT_TLV("Left Digital Gain",
			   MAX98512_R0035_AMP_VOL_CTRL,
			   0,
			   (1 << MAX98512_AMP_VOL_WIDTH) - 1, 0,
			   max98512_left_digital_gain_get,
			   max98512_left_digital_gain_put,
			   max98512_digital_tlv),
			      ...
			      const struct i2c_device_id *id)
{
	...
	if (i2c->dev.of_node) {
		...
		if (of_property_read_u32(i2c->dev.of_node,
					 "maxim,digital-gain-left",
					 &max98512->digital_gain_left)) {
			dev_warn(&i2c->dev, "set digital_gain_left by default.\n");
			max98512->digital_gain_left = 0x40; /* 0db */
		}
