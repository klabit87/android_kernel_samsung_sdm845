...
static void wdsp_glink_tx_buf_work(struct work_struct *work)
{
	...
	dev_info(wpriv->dev, "%s: ch name = %s, payload size = %d\n",
	...
	if (ch->channel_state == GLINK_CONNECTED) {
		...
		dev_info(wpriv->dev, "%s: calling glink_tx\n",
			__func__);
		ret = glink_tx(ch->handle, tx_buf,
			       cpkt->payload, cpkt->payload_size,
			       GLINK_TX_REQ_INTENT);
		dev_info(wpriv->dev, "%s: glink_tx returned %d\n",
			__func__, ret);
		if (ret < 0) {
				...
				size_t count, loff_t *ppos)
{
	...
	case WDSP_CMD_PKT:
		...
		dev_info(wpriv->dev, "%s: requested ch_name: %s, pkt_size: %zd\n",
