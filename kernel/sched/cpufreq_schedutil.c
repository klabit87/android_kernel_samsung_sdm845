...
static void sugov_get_util(unsigned long *util, unsigned long *max, int cpu)
{
	...
	struct sugov_policy *sg_policy = loadcpu->sg_policy;
	...
	*util = cpu_util_freq(cpu, &loadcpu->walt_load);
	if (sg_policy->tunables->legacy_load)
...
static ssize_t legacy_load_show(struct gov_attr_set *attr_set, char *buf)
{
	struct sugov_tunables *tunables = to_sugov_tunables(attr_set);

	return scnprintf(buf, PAGE_SIZE, "%u\n", tunables->legacy_load);
}

static ssize_t legacy_load_store(struct gov_attr_set *attr_set, const char *buf,
				   size_t count)
{
	struct sugov_tunables *tunables = to_sugov_tunables(attr_set);

	if (kstrtobool(buf, &tunables->legacy_load))
		return -EINVAL;
...
static struct governor_attr legacy_load = __ATTR_RW(legacy_load);
		...
		struct sugov_tunables *tunables)
{
	...
	cached->legacy_load = tunables->legacy_load;
...
static void sugov_tunables_restore(struct cpufreq_policy *policy)
{
	...
	tunables->legacy_load = cached->legacy_load;
